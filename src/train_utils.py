...
def load_hparams(args, model_config):
    # Create a HParams object specifying the names and values of the model
    # hyperparameters
    ...
    tf.logging.log(
        tf.logging.INFO, f"Using hyperparameters: {str(hparams.values())}")
...
def load_json_configs(config_file_list, args=None):
    """
    Loads a list of json configuration files into one combined map.
    Configuration files at the end of the list take precedence over earlier
    configuration files (so they will overwrite earlier configs!)

    If args is passed, then this function will attempt to replace entries
    surrounded with the special tokens ## ## with an entry from args with the same name.
    ...
    """
    tf.logging.log(
        tf.logging.INFO, f"train_utils.load_json_configs({config_file_list})")
    combined_config = {}
    if config_file_list:
        ...
        for config_file in config_files:
            tf.logging.log(
                tf.logging.INFO,
                f"train_utils.load_json_configs loading {config_file}")
            if args:
                # read the json in as a string so that we can run a replace on
                # it
                ...
                for match in matches:
                        try:
                            ...
                            json_str = json_str.replace('##%s##' % match,
                                                        value)
                        except AttributeError:
                            tf.logging.log(
                                tf.logging.ERROR,
                                f'Could not find "{match}" attribute in '
                                f'command line args when parsing: '
                                f'{config_file}')
                            sys.exit(1)
                try:
                    ...
                    tf.logging.log(
                        tf.logging.ERROR, f'Error reading json: '
                        f'"{config_file}"')
...
def copy_without_dropout(hparams):
    new_hparams = {
        k: (1.0 if 'dropout' in k else v) for k, v in hparams.values().items()
        }
...
def get_vars_for_moving_average(average_norms):
    ...
    if not average_norms:
        vars_to_average = [
            v for v in tf.trainable_variables() if 'norm' not in v.name]
    tf.logging.log(tf.logging.INFO,
                   "Creating moving averages for "
                   f"{len(vars_to_average)} variables.")
...
def learning_rate(hparams, global_step):
    ...
    if warmup_steps > 0:
        ...
        lr *= tf.minimum(tf.rsqrt(global_step_float),
                     tf.multiply(global_step_float,
                                 warmup_steps ** -decay_rate))
        return lr
    else:
